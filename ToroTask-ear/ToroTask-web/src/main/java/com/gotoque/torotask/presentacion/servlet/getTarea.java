...
public class getTarea extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			Vector vectorTareas = (Vector) oBDWFMotorConsulta
					.getConsultarIdTarea(hParameters);
			Vector vectorComentarios = oBDWFMotorConsulta
					.getConsultarComentarios(hParameters);
			Vector vectorAdjuntos = oBDWFMotorConsulta
					.getConsultarAdjuntos(hParameters);
			Vector vectorIntegrantes = oBDWFMotorConsulta
					.getConsultaIntegrantesxTarea(hParameters);

			request.setAttribute("tarea", new JSONArray(vectorTareas)
					.getJSONArray(0).getJSONObject(0));
			request.setAttribute("comentarios",
					new JSONArray(vectorComentarios));
			request.setAttribute("adjuntos", new JSONArray(vectorAdjuntos));
			request.setAttribute("integrantes",
					new JSONArray(vectorIntegrantes));
			if (online.equals("true")) {
				dispatcher = getServletContext().getRequestDispatcher(
						"/jsp/pdf/printTarea.jsp");
			} else {
				dispatcher = getServletContext().getRequestDispatcher(
						"/jsp/pdf/pdfTarea.jsp");
			}
		} catch (Exception e) {
			e.printStackTrace();
			dispatcher = getServletContext().getRequestDispatcher(
